...
int main( int argc, char *argv[]) {
  ...
  fstat(0, &fileStats);
  char *buffer;
  int regFile = S_ISREG(fileStats.st_mode);
  if (regFile)
  ...
  int cc=0, nc=0, k; /* cc==character count, nc==newline count */
  char c;
  if ( regFile && read(0, buffer, bsize-1)>0)
    {
      cc = bsize-1;
      for (k = 0; k < cc; k++)
	if (buffer[k] == '\n') nc++;
    }
  else if (read(0, buffer, bsize-1)<0)
    {
